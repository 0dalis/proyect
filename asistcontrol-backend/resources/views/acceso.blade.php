            ...
            <div class="form-panel panel-contacto w-full md:w-1/2 p-6 sm:p-10 flex flex-col justify-center" id="mobilePanelContacto">
                <form id="contactoForm" class="space-y-3.5">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Déjanos tus datos y te contactamos vía WhatsApp.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="tel" name="telefono" required class="form-input" placeholder="+52 55 1234 5678">
                    </div>

                    @php
                        $planSeleccionado = request()->get('plan');
                        $mensajePredefinido = $planSeleccionado
                            ? 'Hola, estoy muy interesado en el plan ' . $planSeleccionado . ', ¿me podrían proporcionar más información al respecto? Me gustaría conocer más sobre sus características y beneficios.'
                            : '';
                    @endphp
                    <div>
                        <label class="block text-[11px] font-medium text-slate-700 dark:text-slate-300 mb-1">Mensaje</label>
                        <textarea name="mensaje" rows="3" class="form-input resize-none" placeholder="Cuéntanos las necesidades de tu empresa...">{{ $mensajePredefinido }}</textarea>
                    </div>
                    ...
                    @php $waNumber = config('app.whatsapp_number'); @endphp
                    @if($waNumber)
                    <div class="pt-3 border-t border-slate-200 dark:border-slate-800">
                        <p class="text-[11px] text-slate-400 dark:text-slate-500 text-center mb-2">o contáctanos directamente</p>
                        <a href="https://wa.me/{{ $waNumber }}?text={{ urlencode('Hola, me gustaría saber más sobre AsistControl') }}"
                           target="_blank"
                           class="w-full inline-flex items-center justify-center gap-2 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white font-semibold text-xs rounded-lg shadow-md shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
